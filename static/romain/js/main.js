(function ($) {
        ...
        for(var i=0; i<input3.length; i++) {
            ...
            // Ajouter supprimer salle ici
                ...
                Salle = {
                    ...
                    var token=$('#token').val();
                    var url ="http://localhost:8000/supprimerSalle?token={token}".replace('{token}',token);
                    $.ajax({
                        url: url,
                        type: "POST",
                        dataType: "json",
    ...
    function Add_Room(){
        ...
        var url = "http://localhost:8000/ajouterSalle?token={token}";
        var data = {
            "Salle":Salle,
        ...
        $.ajax({
            url : "http://localhost:8000/ajouterSalle?token={token}".replace('{token}', token),
            type: "POST",
            dataType: "json",
            data: data,
