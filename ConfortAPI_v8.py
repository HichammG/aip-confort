...
def MADM(Temperature, Acoustique, Luminosite, CO2, Humidite ):
    ...
    mf.rules.append(mf.parse_rule('if (input1 is TemperatureOptimale) and (input2 is BruitOptimal) and (input3 is LuminositeOptimale) and (input4 is CO2Optimal) and (input5 is HumiditeOptimale) then (output is Confortable)'))
    mf.rules.append(mf.parse_rule('if (input1 is not TemperatureOptimale) or (input2 is not BruitOptimal) or (input3 is not LuminositeOptimale) or (input4 is not CO2Optimal) or (input5 is not HumiditeOptimale)then (output is Inconfortable)'))
            ...
            try:
                for i in range(int(len(salle))):
                    ...
                    for j in range(nbRaspSalle[i]):
                            ...
                            for k in range(int(len(ip))):
                                ...
                                if salle[i] in str(rows):
                                    if ip[k] in str(rows):
                                        ...
                                        EtatCapteurs.append([EtatCapteurTemperature,EtatCapteurAcoustique,EtatCapteurLuminosite,EtatCapteurCO2,EtatCapteurHumidite])
                        ...
                        for k in range(int(len(ip))):
                            for r in db[salle[i]].find({'ip':ip[k]},{'_id':0}).sort('Time',-1).limit(5):
                                ...
                                if (StatutTemperature == True and EtatCapteurs[j][0] == True and 'temperature' in r['capteur temperature']):
                                ...
                                if (StatutAcoustique == True and EtatCapteurs[j][1] == True and 'bruit' in r['capteur bruit']):
                                    ValeursAcoustique.append(r['capteur bruit']['bruit']['bruit'])
                                else:
                                    ValeursAcoustique.append(None)
                                    ValeurNulle[1] = ValeurNulle[1] + 1

                                if (StatutLuminosite == True and EtatCapteurs[j][2] == True and 'luminosite' in r['capteur luminosite']):
                                ...
                                if (StatutHumidite == True and EtatCapteurs[j][4] == True and 'humidite' in r['capteur humidite']):
                    ...
                    if AnyData == False:
                        ...
                        if demandeTemperature == True:
                            if int(len(ValeursTemperature_SansNone)) > ValeurNulle[0]:
                                if (int(len(ValeursTemperature_SansNone))-ValeurNulle[0]) != 0:
                                    Temperature = sum(ValeursTemperature_SansNone)/(int(len(ValeursTemperature_SansNone))-ValeurNulle[0])
                        if Temperature == None:
                            Temperature = 21
                        ...
                        if demandeAcoustique == True:
                            if int(len(ValeursAcoustique_SansNone)) > ValeurNulle[1]:
                                if (int(len(ValeursAcoustique_SansNone)) - ValeurNulle[1]) != 0:
                                    Acoustique = sum(ValeursAcoustique_SansNone) / (int(len(ValeursAcoustique_SansNone)) - ValeurNulle[1])
                        if Acoustique == None:
                            Acoustique = 0
                        ...
                        if demandeLuminosite == True:
                            if int(len(ValeursLuminosite_SansNone)) > ValeurNulle[2]:
                                if (int(len(ValeursLuminosite_SansNone)) - ValeurNulle[2]) != 0:
                                    Luminosite = sum(ValeursLuminosite_SansNone) / (int(len(ValeursLuminosite_SansNone)) - ValeurNulle[2])
                        ...
                        if demandeHumidite == True:
                            if int(len(ValeursLuminosite_SansNone)) > ValeurNulle[4]:
                                if (int(len(ValeursHumidite_SansNone)) - ValeurNulle[4]) != 0:
                                    Humidite = sum(ValeursHumidite_SansNone) / (int(len(ValeursHumidite_SansNone)) - ValeurNulle[4])
                        ...
                        if Materiel == None:
                            Materiel = []
                        sortRes.append(({salle[i]:{'Note':Note,'Informations Problemes' : "Any value",'Materiel':Materiel}},Note))
...
code = 500
    ...
    if User != None:
        try:
            res = db['Capteurs'].update_one({data['Salle']: {"$exists": True}}, {"$set": {data['Salle']: [{'Temperature': bool(data['Temperature']),
